...
/** Same convention as [evola.composeapp.main.HomeContainerTest]: a real [LocalLessonsRepository]
 ...
 * exercised indirectly by every other Container that shares this exact poll-until-terminal shape. */
class LessonDetailContainerTest {
    ...
    fun `a ready lesson loads with its real vocabulary count and section list`() = runTest {
        LessonDetailContainer("l1", setup(vocabCount = 3)).store.subscribeAndTest {
            val loaded = assertIs<LessonDetailState.Loaded>(states.first { it !is LessonDetailState.Loading })
    ...
    fun `an unknown lesson id surfaces as Error, not a crash`() = runTest {
        LessonDetailContainer("does-not-exist", setup()).store.subscribeAndTest {
            val error = assertIs<LessonDetailState.Error>(states.first { it !is LessonDetailState.Loading })
