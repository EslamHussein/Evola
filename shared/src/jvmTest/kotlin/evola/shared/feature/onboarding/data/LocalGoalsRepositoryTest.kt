...
class LocalGoalsRepositoryTest {
    ...
    private fun repo(): Pair<LocalGoalsRepository, AppDatabase> {
    ...
    fun `progress reflects a fully mastered lesson and streak`() = runTest {
        ...
        db.materialDao().insert(MaterialEntity("m1", LOCAL_USER, goal.id, "f.pdf", "h", "READY", "application/pdf", 1L, null, "entire", null, null, "txt", 0L, 0L, 0L))
        db.lessonDao().insert(LessonEntity("l1", "m1", goal.id, 1L, "Lesson 1", "ready", "curriculum", null, null, 0L))
        db.vocabularyDao().insertItem(VocabularyItemEntity("v1", "l1", "Hund", "dog", null, null, null, null, null, null, null, null, null, null, null, null, null, null, 0L))
        db.vocabularyDao().insertProgress(VocabularyProgressEntity("p1", LOCAL_USER, "v1", "mastered", 0L, 0L, 0L, 0L, null, 0L, 0L))
        db.activityDao().upsert(DailyActivityEntity("a1", LOCAL_USER, "2026-08-05", 1L))
    ...
    fun `mid-ladder status yields partial progress`() = runTest {
        ...
        db.vocabularyDao().insertProgress(VocabularyProgressEntity("p1", LOCAL_USER, "v1", "learning", 0L, 0L, 0L, 0L, null, 0L, 0L))
    ...
    fun `weekly activity and today's learned count reflect completed sessions`() = runTest {
        ...
        db.materialDao().insert(MaterialEntity("m1", LOCAL_USER, goal.id, "f.pdf", "h", "READY", "application/pdf", 1L, null, "entire", null, null, "txt", 0L, 0L, 0L))
        db.lessonDao().insert(LessonEntity("l1", "m1", goal.id, 1L, "Lesson 1", "ready", "curriculum", null, null, 0L))
        db.settingsDao().upsert(UserSettingEntity(LOCAL_USER, "daily_new_word_goal", "9"))

        db.vocabularyDao().insertSession(VocabularySessionEntity("s1", LOCAL_USER, "l1", 1L, 0L, null, null, 6L, 2L, 0L, 0L))
        db.vocabularyDao().completeSession(0L, "2026-08-05", "s1")
        db.activityDao().upsert(DailyActivityEntity("a1", LOCAL_USER, "2026-08-05", 1L))
