...
class LocalGoalsRepositoryTest {
    ...
    @Test
    fun `progress reflects a mastered vocab item and streak`() = runTest {
        val (r, db) = repo()
        val goal = (r.createGoal("Learn German", null) as CreateGoalResult.Success).goal
        // one material + lesson + a mastered vocab item
        ...
        db.vocabularyQueries.insertProgress("p1", LOCAL_USER, "v1", "mastered", 6L, 4L, 0L, null, 0L, 0L)
